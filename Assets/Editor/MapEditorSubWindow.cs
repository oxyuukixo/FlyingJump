            ...
            if (m_TargetMap.m_MapChipObject.Count > 0 && m_MainWindow.m_NewMapChipNum != null)
            {
                for (int y = 0; y < m_MainWindow.m_NewNumY; y++)
                {
                    for (int x = 0; x < m_MainWindow.m_NewNumX; x++)
                    {
                        ...
                        GUI.DrawTexture(new Rect(area.x + x * m_MainWindow.m_NewSizeX, area.y + y * m_MainWindow.m_NewSizeY, m_MainWindow.m_NewSizeX, m_MainWindow.m_NewSizeY),
                            m_TargetMap.m_MapChipObject[m_MainWindow.m_NewMapChipNum[y].List[x]].GetComponent<SpriteRenderer>().sprite.texture);
                    }
                }
            }
